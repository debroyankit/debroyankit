...
ROWS = [
    ("host",),
    ("kv", "Now", "Software Engineer @ Sigmoid"),
    
    ("kv", "Edu", "B.Tech ECE, NITS' 25"),
    ("gap",),
    ("sec", "Stack"),
    ("kv", "Languages", "C++, Java, Python, Kotlin, SQL"),
    ("kv", "Databases", "MySQL, PostgreSQL, MongoDB, Redis, FAISS"),
    ("kv", "Backend", "Spring Boot, FastAPI, RESTful APIs, gRPC, WebSockets"),
    ("kv", "AI / ML", "LangChain, PyTorch, TensorFlow, RAG, Pandas"),
    ("kv", "Tools", "AWS, Docker, Kubernetes, GitHub Actions, CI/CD"),
    ("gap",),
    ("sec", "About me"),
    ("bul", "Passionate about building scalable and efficient systems"),
    ("bul", "Loves to learn new technologies and work on challenging projects")
]
...
parts.append(f'<text x="{W/2}" y="{TITLEBAR_H/2 + 4}" fill="{MUTED}" font-size="12" '
             f'text-anchor="middle">ankit@github: ~$ neofetch</text>')
...
for i, row in enumerate(ROWS):
    ...
    if kind == "host":
        inner = (f'<text x="{KEY_X}" y="{y:.1f}" font-size="14" font-weight="700">'
                 f'<tspan fill="{GREEN}">ankit</tspan><tspan fill="{MUTED}">@</tspan>'
